...
### 2026-03-29 — Email attachment direct download

- **Gmail is the filesystem.** Excom's pointer-based architecture means email bodies and attachments live in Gmail, not in the Frappe database or server filesystem. Downloading an attachment should stream bytes from Gmail → server memory → browser — never touching disk. This avoids storage costs, keeps the server stateless for email content, and ensures attachments reflect the live Gmail state (if an attachment is removed in Gmail, Excom won't serve a stale copy).
