...
export function SettingsPage({ onNavigateBack }: SettingsPageProps) {
  ...
  return (
    <div className="h-full min-h-0 w-full bg-white text-zinc-900 flex flex-col">
      <header className="shrink-0 border-b border-zinc-200 px-3 py-2 flex items-center gap-3 bg-zinc-50/80">
        <Button
          variant="ghost"
          size="icon"
      ...
      <div className="flex flex-1 min-h-0">
        {/* Left nav */}
        <nav className="w-52 shrink-0 border-r border-zinc-200 bg-zinc-50/40 flex flex-col py-2 overflow-y-auto">
          {NAV_ITEMS.map((item) => (
            <button
              key={item.id}
              ...
              className={`flex items-center gap-2.5 px-3 py-2.5 text-sm font-medium transition-colors text-left ${
                activeSection === item.id
                  ? "bg-blue-500/10 text-blue-700 border-r-2 border-blue-500"
...
function GeneralSection({ siteUrl: _siteUrl }: { siteUrl: string }) {
  ...
  return (
    <div className="max-w-2xl space-y-4">
      <SectionHeader icon={<Zap className="w-4 h-4 text-blue-700" />} title="General" />
      ...
      <Card>
        ...
        {mobile?.can_manage_mobile_oauth ? (
          <div className="mt-5 flex flex-wrap gap-2 items-center">
            ...
            <a href="/app/excom-settings/Excom%20Settings" className="inline-flex items-center gap-1 text-xs text-blue-700 hover:text-blue-700">
              Excom Settings <ExternalLink className="w-3 h-3" />
            </a>
          </div>
        ) : (
          <p className="mt-4 text-xs text-zinc-600 flex items-start gap-2">
            <Shield className="w-4 h-4 shrink-0 mt-0.5 text-zinc-500" />
...
function SignaturesSection() {
  ...
  return (
    <div className="max-w-2xl space-y-4">
      <SectionHeader icon={<Mail className="w-4 h-4 text-emerald-700" />} title="Email Signatures" />
      <Card>
        <p className="text-sm text-zinc-600 mb-3">
          Your signature is appended to outgoing emails. Supports HTML formatting.
        </p>
        <div className="mb-3">
          <label className="block text-xs text-zinc-600 mb-1 uppercase tracking-wide">Position</label>
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-sm border transition-colors ${
                  position === opt
                    ...
                    : "border-zinc-300 text-zinc-600 hover:border-zinc-300 hover:text-zinc-900"
                }`}
              >
                {opt}
              </button>
        ...
        <div className="mb-3">
          <label className="block text-xs text-zinc-600 mb-1 uppercase tracking-wide">Signature (HTML)</label>
          <textarea
            value={html}
            onChange={(e) => setHtml(e.target.value)}
            rows={8}
            className="w-full bg-zinc-100/60 border border-zinc-300 rounded-lg px-3 py-2 text-sm text-zinc-900 font-mono resize-y focus:outline-none focus:border-blue-500 placeholder:text-zinc-500"
            placeholder="<p>Best regards,<br><strong>Your Name</strong></p>"
          />
        </div>
        {html && (
          <div className="mb-3">
            <label className="block text-xs text-zinc-600 mb-1 uppercase tracking-wide">Preview</label>
            <div
              className="border border-zinc-300 rounded-lg p-3 text-sm text-zinc-700 bg-zinc-100/40"
...
function NotificationsSection() {
  ...
  return (
    <div className="max-w-2xl space-y-4">
      <SectionHeader icon={<Bell className="w-4 h-4 text-amber-700" />} title="Notifications" />

      <Card>
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="font-medium text-zinc-900 text-sm">Notification sound</p>
            <p className="text-xs text-zinc-600 mt-0.5">Play a tone when a new message arrives, even when this tab is focused.</p>
          </div>
          <button
      ...
      <Card>
        ...
        <div className="flex flex-wrap items-center gap-3 mb-3 text-sm">
          ...
          <span className={notifPerm === "granted" ? "text-emerald-700" : notifPerm === "denied" ? "text-red-700" : "text-amber-700"}>
            {permLabel}
          </span>
          <span className="text-zinc-500">·</span>
          <span className="text-zinc-600">Server:</span>
          <span className={serverPushEnabled ? "text-emerald-700" : "text-zinc-600"}>
            {serverPushEnabled ? "On" : "Off"}
          </span>
          <span className="text-zinc-500">·</span>
          <span className="text-zinc-600">This browser:</span>
          <span className={fcmRegistered ? "text-emerald-700" : "text-zinc-600"}>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {fcmRegistered && (
            <Button variant="outline" onClick={() => void handleDisablePush()} disabled={pushBusy} className="border-zinc-300 text-zinc-700">
...
function BrandingSection({ branding }: { branding: ReturnType<typeof useExcomBranding>["branding"] }) {
  return (
    <div className="max-w-2xl space-y-4">
      <SectionHeader icon={<Palette className="w-4 h-4 text-pink-700" />} title="Branding" />
      <Card>
        <p className="text-sm text-zinc-600 mb-3">
          Logo gradient, app name, and colours are managed in{" "}
          <a href="/app/excom-settings/Excom%20Settings" className="text-blue-700 hover:text-blue-700 inline-flex items-center gap-1">
            Excom Settings <ExternalLink className="w-3 h-3" />
          </a>{" "}
          on the desk.
        </p>
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            {branding?.show_app_name ? (
              <p className="text-sm text-zinc-700 font-medium truncate">Header title: {branding.app_name || "Excom"}</p>
            ) : (
              <p className="text-sm text-zinc-600">App name in header is hidden. Enable "Show App Name in Header" in Excom Settings.</p>
            )}
          </div>
        </div>
      </Card>
...
function AccountsSection() {
  ...
  return (
    <div className="max-w-2xl space-y-4">
      <SectionHeader icon={<Users className="w-4 h-4 text-cyan-700" />} title="Accounts" />
      <Card>
        <p className="text-sm text-zinc-600 mb-3">Channel accounts you have access to.</p>
        {accounts.length === 0 ? (
          <p className="text-sm text-zinc-500">No accounts available.</p>
        ) : (
          <ul className="divide-y divide-zinc-200">
            {accounts.map((acc) => (
              <li key={acc.name} className="py-2 flex items-center justify-between gap-3">
                <div>
                  <p className="text-sm font-medium text-zinc-900">{acc.account_name}</p>
                  <p className="text-xs text-zinc-600 mt-0.5">{acc.email_address || acc.wa_phone_id || acc.name}</p>
                </div>
                <span className={`text-[11px] px-2 py-0.5 rounded-full border font-medium ${CHANNEL_BADGE[acc.channel] || "bg-zinc-200/30 text-zinc-600 border-zinc-300"}`}>
                  {acc.channel}
                </span>
              </li>
            ))}
...
function ShortcutsSection() {
  return (
    <div className="max-w-2xl space-y-4">
      <SectionHeader icon={<KeyRound className="w-4 h-4 text-purple-700" />} title="Keyboard Shortcuts" />
      <Card>
        <p className="text-sm text-zinc-600 mb-3">Available keyboard shortcuts for power users.</p>
              ...
              <tr key={i} className="py-2">
                <td className="py-2.5 pr-4 w-40">
                  <div className="flex items-center gap-1">
                    {keys.map((k) => (
                      <kbd key={k} className="inline-flex items-center px-1.5 py-0.5 rounded border border-zinc-300 bg-zinc-100 text-xs font-mono text-zinc-700">
                        {k}
                      </kbd>
                    ))}
                  </div>
                </td>
                <td className="py-2.5 text-zinc-700">{desc}</td>
              </tr>
            ))}
          </tbody>
        </table>
        <p className="text-xs text-zinc-500 mt-4">Use Ctrl instead of ⌘ on Windows/Linux.</p>
      </Card>
    </div>
  );
}
...
function CannedSection() {
  ...
  return (
    <div className="max-w-2xl space-y-4">
      <SectionHeader icon={<MessageSquare className="w-4 h-4 text-indigo-700" />} title="Canned Responses" />
      <Card>
        <p className="text-sm text-zinc-600 mb-3">Quick-reply templates triggered by typing <code className="text-zinc-700">/shortcode</code> in chat.</p>
        {(items?.length ?? 0) > 0 && (
          <ul className="divide-y divide-zinc-200 mb-3">
            {items!.map((item) => (
              <li key={item.name} className="py-2.5">
                <div className="flex items-center gap-2">
                  <p className="text-sm font-medium text-zinc-900">{item.title}</p>
                  <span className="text-xs text-zinc-600 font-mono">/{item.shortcode}</span>
                </div>
                <p className="text-xs text-zinc-600 mt-0.5 line-clamp-2">{item.content}</p>
              </li>
            ))}
          </ul>
        )}
        <div className="border-t border-zinc-200 pt-4 space-y-2">
          <p className="text-xs text-zinc-600 uppercase tracking-wide mb-2">Add new</p>
          <input
            value={newTitle}
            onChange={(e) => setNewTitle(e.target.value)}
            placeholder="Title (e.g. Greeting)"
            className="w-full bg-zinc-100/60 border border-zinc-300 rounded-lg px-3 py-2 text-sm text-zinc-900 focus:outline-none focus:border-blue-500"
          />
          <input
            value={newShortcode}
            onChange={(e) => setNewShortcode(e.target.value)}
            placeholder="Shortcode (e.g. greeting) — auto-generated from title if empty"
            className="w-full bg-zinc-100/60 border border-zinc-300 rounded-lg px-3 py-2 text-sm text-zinc-900 font-mono focus:outline-none focus:border-blue-500"
          />
          <textarea
            value={newContent}
            ...
            rows={3}
            className="w-full bg-zinc-100/60 border border-zinc-300 rounded-lg px-3 py-2 text-sm text-zinc-900 resize-y focus:outline-none focus:border-blue-500"
          />
          <Button onClick={() => void handleAdd()} disabled={saving} size="sm" className="bg-blue-500/20 text-blue-700 border border-blue-500/30 hover:bg-blue-500/30">
...
function AutoReplySection() {
  return (
    <div className="max-w-2xl space-y-4">
      <SectionHeader icon={<RotateCcw className="w-4 h-4 text-teal-700" />} title="Auto-Reply" />
      <Card>
        <p className="text-sm text-zinc-600 mb-2">
          Configure automatic replies per channel account — e.g., out-of-office messages, welcome messages.
        </p>
        <p className="text-xs text-zinc-500 p-3 rounded-lg bg-zinc-100/40 border border-zinc-300/50">
          Auto-reply rule management coming soon. For now, configure rules via the Frappe desk under{" "}
          <a href="/app" className="text-blue-700 hover:text-blue-700">Excom modules</a>.
        </p>
      </Card>
    </div>
  );
...
function InfoRow({
  ...
  return (
    <div>
      <dt className="text-zinc-600 text-xs uppercase tracking-wide mb-1">{label}</dt>
      <dd className="flex flex-wrap items-center gap-2">
        <code className="text-zinc-900 bg-zinc-100 px-2 py-1 rounded break-all text-xs">
          {value || "—"}
        </code>
        {value && onCopy && (
          <Button type="button" variant="ghost" size="icon" className="h-7 w-7 text-zinc-600" onClick={() => onCopy(value)}>
