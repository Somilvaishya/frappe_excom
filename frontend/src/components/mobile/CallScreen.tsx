...
export function CallScreen({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gradient-to-b from-zinc-900 via-zinc-950 to-black flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6">
        {callType === "video" && callStatus === "active" ? (
          <div className="w-full max-w-sm aspect-[3/4] bg-zinc-900 rounded-2xl overflow-hidden relative mb-6">
            <div className="absolute inset-0 bg-gradient-to-br from-blue-500/20 to-purple-500/20 flex items-center justify-center">
              <div className="text-center">
                <Video className="w-16 h-16 text-white/50 mx-auto mb-2" />
                <p className="text-white/50 text-sm">Video feed</p>
              </div>
            </div>
            <div className="absolute top-4 right-4 w-24 h-32 bg-zinc-800 rounded-lg overflow-hidden border-2 border-zinc-700">
              {contact.avatar && (
                <img
                  src={contact.avatar}
                  alt="You"
            ...
            <div className="w-32 h-32 mx-auto mb-6 relative">
              {contact.avatar ? (
                <img
                  src={contact.avatar}
                  alt={contact.name}
                  className="w-full h-full rounded-full object-cover ring-4 ring-zinc-800"
                />
              ) : (
                <div className="w-full h-full rounded-full bg-gradient-to-br from-blue-500 to-purple-600 ring-4 ring-zinc-800 flex items-center justify-center text-white text-3xl font-medium">
                  {contact.name[0]}
                </div>
              )}
              {callStatus === "active" && (
        ...
        {contact.company && (
          <p className="text-zinc-400 mb-4">{contact.company}</p>
        )}
