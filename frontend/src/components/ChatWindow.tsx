...
export default function ChatWindow() {
  ...
  const { data: threadData } = useFrappeGetCall<{ message: ExcomThread }>(
    threadId ? "frappe.client.get" : null,
    threadId ? { doctype: "Excom Thread", name: threadId } : undefined
  );

  const thread = threadData?.message;
  const { messages, isLoading, refresh } = useMessages(threadId || "");
  ...
  async function handleSend() {
    ...
    setSending(true);
    try {
      await call({ thread_id: threadId, message: text });
      refresh();
    } catch (err: any) {
  ...
  const displayName = thread?.display_name || threadId;
  const phone = thread?.primary_phone || "";
    ...
    <div className="flex-1 flex flex-col h-full bg-chat-bg relative">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-chat-header z-10 border-b border-chat-border/30">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-[16px] font-normal text-white leading-tight">{displayName}</h2>
            {phone && <p className="text-xs text-chat-muted">{phone}</p>}
          </div>
        </div>
        <div className="flex items-center gap-1">
          <button className="p-2 rounded-full hover:bg-chat-hover transition-colors text-chat-muted"><FiSearch size={20} /></button>
          <button className="p-2 rounded-full hover:bg-chat-hover transition-colors text-chat-muted"><HiOutlineDotsVertical size={20} /></button>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto py-2 z-10">
        {isLoading ? (
          <div className="flex items-center justify-center h-full text-chat-muted text-sm">Loading messages...</div>
        ) : messages.length === 0 ? (
          <div className="flex items-center justify-center h-full">
            <div className="bg-chat-panel text-chat-muted text-sm px-4 py-2 rounded-lg shadow-sm text-center">No messages yet. Start a conversation!</div>
          </div>
        ) : (
          <>
            {messages.map((msg, idx) => {
              const prevMsg = idx > 0 ? messages[idx - 1] : null;
              const showDate = !prevMsg || !dayjs(msg.creation).isSame(dayjs(prevMsg.creation), "day");
              return <MessageBubble key={msg.name} message={msg} showDate={showDate} />;
            })}
      ...
      <div className="flex items-end gap-2 px-4 py-3 bg-chat-header z-10 border-t border-chat-border/30">
        <button className="p-2 rounded-full hover:bg-chat-hover transition-colors text-chat-muted shrink-0 mb-0.5"><BiPlus size={24} /></button>
        <button className="p-2 rounded-full hover:bg-chat-hover transition-colors text-chat-muted shrink-0 mb-0.5"><BiSmile size={24} /></button>
